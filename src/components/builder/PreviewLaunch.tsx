...
import {
  Globe, ExternalLink, Pencil, Check, ArrowLeft, Rocket, Copy, Zap,
} from 'lucide-react';
...
export default function PreviewLaunch({ project, onLaunch, onBack }: PreviewLaunchProps) {
  const navigate = useNavigate();
  const [copied, setCopied] = useState(false);
  const [domain, setDomain] = useState('');

  const previewUrl = `https://preview.siteforge.ai/p/${project?.slug || 'demo'}`;
  ...
  const handleEdit = () => {
    if (project?.id) navigate(`/editor/${project.id}`);
  };
    ...
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-8">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-green-500/10 border border-green-500/20 mb-4">
          ...
          <span className="text-xs font-medium text-green-300 tracking-wider uppercase">Ready to Launch</span>
        </div>
        <h2 className="text-2xl sm:text-3xl font-bold text-white mb-2" style={{ fontFamily: 'Oswald, Inter, sans-serif' }}>
          Your Website is Ready
        </h2>
        <p className="text-sm text-white/40">
          {project?.industry} website generated for &quot;{project?.businessName || project?.name}&quot;.
        </p>
      </div>
      ...
      <div className="mb-8 rounded-2xl border border-white/5 bg-white/[0.02] overflow-hidden">
        <div className="flex items-center gap-2 px-4 py-3 border-b border-white/5 bg-white/[0.03]">
          ...
          <div className="flex-1 mx-4">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 text-xs text-white/30">
              ...
              <button onClick={handleCopy} className="ml-auto p-1 rounded hover:bg-white/5 transition-colors">
        ...
        <div className="relative aspect-[16/10] bg-gradient-to-br from-violet-500/10 to-blue-500/10 flex items-center justify-center overflow-hidden">
          <img src="/images/hero-website.jpg" alt="Website Preview" className="absolute inset-0 w-full h-full object-cover opacity-70" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-black/20" />
          <div className="absolute top-4 right-4 flex flex-col gap-2">
            <div className="liquid-glass px-3 py-1.5 rounded-lg">
              <div className="text-[10px] text-white/50 uppercase">Sections</div>
              <div className="text-sm font-bold text-white">{Array.isArray(project?.sections) ? project.sections.length : 5}</div>
            </div>
            <div className="liquid-glass px-3 py-1.5 rounded-lg">
      ...
      <div className="grid sm:grid-cols-3 gap-3 mb-8">
        <button onClick={handleEdit}
          ...
          <Pencil className="w-4 h-4" /><span>Edit Design</span>
        </button>
        <button onClick={() => window.open(previewUrl, '_blank')}
          className="flex items-center justify-center gap-2 py-3 rounded-xl border border-white/10 bg-white/[0.02] text-white font-medium text-sm hover:bg-white/[0.05] transition-all">
          <ExternalLink className="w-4 h-4" /><span>Open Preview</span>
        </button>
        <button onClick={() => alert('Code export coming soon!')}
          className="flex items-center justify-center gap-2 py-3 rounded-xl border border-white/10 bg-white/[0.02] text-white font-medium text-sm hover:bg-white/[0.05] transition-all">
          <Zap className="w-4 h-4" /><span>Export Code</span>
        </button>
      </div>
      ...
      <div className="p-6 rounded-2xl border border-white/5 bg-white/[0.02] mb-8">
        <h3 className="text-sm font-semibold text-white mb-4 flex items-center gap-2">
          <Globe className="w-4 h-4 text-violet-400" />Connect Custom Domain
        </h3>
        <div className="flex gap-3">
          ...
          <button onClick={() => alert('Domain connected! (Demo)')}
            className="px-5 py-3 rounded-xl text-white font-medium text-sm transition-all hover:shadow-lg hover:shadow-violet-500/25"
            style={{ background: 'linear-gradient(135deg, #6b46c1 0%, #2563eb 100%)' }}>Connect</button>
        </div>
        <p className="mt-2 text-xs text-white/30">Free SSL certificate included. DNS changes may take up to 24 hours to propagate.</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <button onClick={onLaunch}
          ...
          <Rocket className="w-4 h-4" /><span>Launch & Edit</span>
        </button>
      </div>
    </div>
  );
